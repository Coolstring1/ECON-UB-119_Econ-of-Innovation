...
ggplot(startups_w) +
  ...
  ylab("Startups founded\n 
       per researcher/per worker outside universities") +
  ggtitle("University versus non-university start-ups, per worker") +
...
ggplot(patents_w, aes(x=appyr)) +
  ...
  scale_color_identity(name = "", 
                       breaks = c("royalblue", "darkorange2"), 
                       labels = c("Patents by non-Univ. workers", "Patents by University workers"), 
                       guide = "legend") +
  theme_bw() +
  ...
  scale_y_continuous(
    ...
    name = "University patents, per university researcher",
    
    # Add a second axis and specify its features
    sec.axis = sec_axis(~./52,name="Non-university patents, per Norwegian worker")
  ) + 
